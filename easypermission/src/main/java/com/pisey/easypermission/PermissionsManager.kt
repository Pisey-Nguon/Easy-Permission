...
fun FragmentActivity.runWithPermissions(
    ...
    return runWithPermissionsHandler(this, forceGranted = forceGranted, permissions = permissions, callback =  callback)
}
...
fun Fragment.runWithPermissions(
    ...
    runWithPermissionsHandler(this, permissions = permissions,callback = callback,forceGranted = forceGranted)
}
...
fun FragmentActivity.runWithLocationPermission(forceGranted: Boolean = true,callback: () -> Unit) {
    runWithPermissionsHandler(this, Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION, forceGranted = forceGranted ){
...
fun Fragment.runWithLocationPermission( forceGranted: Boolean = true,callback: () -> Unit) {
    runWithGPSPermissions(callback = {
        runWithPermissionsHandler(this, Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION, callback = callback, forceGranted = forceGranted)
    }, forceGranted = forceGranted)
}

fun FragmentActivity.runWithBackgroundLocationPermission(forceGranted: Boolean = true,callback: () -> Unit) {
    runWithGPSPermissions(callback = {
        runWithPermissionsHandler(this, Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.ACCESS_FINE_LOCATION, forceGranted = forceGranted) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) runWithPermissionsHandler(this, Manifest.permission.ACCESS_BACKGROUND_LOCATION,forceGranted = forceGranted, callback = callback)
            else callback()
        }
    }, forceGranted = forceGranted)
}

fun Fragment.runWithBackgroundLocationPermission(forceGranted: Boolean = true,callback: () -> Unit) {
    runWithGPSPermissions(callback = {
        runWithPermissionsHandler(this, Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.ACCESS_FINE_LOCATION, forceGranted = forceGranted) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) runWithPermissionsHandler(this, Manifest.permission.ACCESS_BACKGROUND_LOCATION, callback = callback, forceGranted = forceGranted)
            else callback()
        }
    }, forceGranted = forceGranted)
}

private fun runWithPermissionGPSHandler(target: Any,forceGranted: Boolean = true, callback: () -> Unit) {
    val permissionCheckerFragment = PermissionCheckerFragment(forceGranted)
    when (target) {
        is FragmentActivity -> {
            target.supportFragmentManager.beginTransaction().apply {
                add(permissionCheckerFragment, PermissionCheckerFragment::class.java.canonicalName).runOnCommit { permissionCheckerFragment.requestPermissionGPSFromUser(callback) }.commitAllowingStateLoss()
            }
...
private fun runWithPermissionsHandler(target: Any, vararg permissions: String,forceGranted: Boolean, callback: () -> Unit) {
